...
from harness.config import ensure_huxley_dirs, resolve_path
...
from harness.daemon.lifecycle import (
    start_daemon,
    stop_daemon,
    daemon_status,
    is_running,
...
def cmd_compact(args):
    ...
    if text is None:
        ...
        return
    cprompt = (
        "Condense this conversation into one paragraph preserving "
    ...
    if not summary:
        print(
            f"γ|compact|err|empty summary|{journal.entry_count()} entries", flush=True
        )
        return
...
def cmd_patch(args):
    ...
    if getattr(args, "list", False) or getattr(args, "rollback", None):
        if args.file or getattr(args, "review", False) or not args.dry_run:
            print(
                "γ|patch|err|--list/--rollback cannot be combined with --apply, --review, or <file>",
                flush=True,
            )
            return 1
    # list known patches
    ...
    if getattr(args, "rollback", None):
        ...
        print(
            f"γ|patch|rollback|{'ok' if ok else 'not_found'}|{args.rollback}",
            flush=True,
        )
    ...
    if sys.stdin.isatty():
        print(
            "γ|patch|err|pipe new content via stdin (e.g., echo 'new_code()' | huxley patch file.py)",
            flush=True,
        )
        return 1
...
def cmd_daemon(args):
    if args.daemon_cmd == "start":
        ...
        if ok:
            ...
            print(
                "γ|huxleyd|start|already_running"
                if is_running()
                else "γ|huxleyd|start|fail",
                flush=True,
            )
...
def cmd_swarm(args):
    if args.swarm_cmd == "peers":
        ...
        for p in data:
            ...
            print(
                f"γ|swarm|peer|{icon}|{p['hostname']:<20}|:{p['port']}|castes={p['castes']}|load={p['load']}|age={p['age']}s{' LOST' if p.get('lost') else ''}",
                flush=True,
            )
        ...
        if data is None:
            ...
            return
        print(
            f"γ|swarm|status|total={data.get('peers', 0)}|active={data.get('active_peers', 0)}",
            flush=True,
        )
        ...
        if r.get("error"):
            ...
            print(
                f"γ|swarm|test|send={r['send_ok']}|recv={r['recv_ok']}|loopback={r['loopback']}",
                flush=True,
            )
        fw = r.get("firewall")
        if fw:
        ...
        if not r.get("loopback"):
            print(
                "γ|swarm|test|hint|no loopback — packets not reaching self", flush=True
            )
            print(
                f"γ|swarm|test|hint|firewall or NECP blocking UDP on port {MULTICAST_PORT}",
                flush=True,
            )
...
def cmd_project(args):
    if args.proj_cmd == "list":
        ...
        for p in projects:
            print(
                f"γ|project|{p['dir']}|{p['title'][:50]}|tasks={p['task_count']}|suggestions={p.get('suggestion_count', 0)}|result={p['result_len']}b",
                flush=True,
            )
        ...
        for t in p.get("tasks", []):
            ...
            print(
                f"γ|project|task|{icon}|{t['id']}|{t['title'][:60]}|units={t['unit_count']}",
                flush=True,
            )
        ...
        try:
            result = materialize_project(
                args.name, destination=args.into, overwrite=args.force
            )
...
def cmd_adev(args):
    from harness.adev import AdversarialDevEngine

    engine = AdversarialDevEngine()
    roles = {
        "programmer": {"caste": args.programmer_caste},
        "roaster": {"caste": args.roaster_caste},
        "adjudicator": {"caste": args.adjudicator_caste},
    }
    print(f"\u03b3|adev|start|{args.task[:60]}", flush=True)
    result = engine.run(
        task=args.task, workdir=args.workdir, max_rounds=args.rounds, **roles
    )
    status = result.get("status", "unknown")
    rounds = result.get("rounds", 0)
    print(f"\u03b3|adev|done|{status}|{rounds} rounds", flush=True)
    code = result.get("code_result", "")
    if code:
        print(f"\u03b3|adev|output|---\n{code[:2000]}", flush=True)
...
def cmd_schedule(args):
    if args.sched_cmd == "list":
        ...
        for s in data:
            ...
            print(
                f"γ|schedule|{icon}|{s['id'][:8]}|{wtype:<10}|{s.get('title', '-')}",
                flush=True,
            )
        ...
        if data is None:
            ...
            return
        print(
            f"γ|schedule|add|{data['id'][:12]}|{args.when_type}|{args.title}",
            flush=True,
        )
        ...
        if data is None:
            ...
            return
        print(
            f"γ|schedule|remove|{'ok' if data.get('status') == 'removed' else 'not_found'}",
            flush=True,
        )
        ...
        for e in data[-10:]:
            print(
                f"γ|schedule|history|{e['schedule_id'][:8]}|{e['action'].get('type', '?')}|{e.get('result', '?')}|{e.get('at', '')[:19]}",
                flush=True,
            )
    ...
    if args.board_cmd == "list":
        ...
        if args.serve_cmd == "start":
            ...
            if not ok:
                st = boardd_status()
                print(
                    f"γ|boardd|start|already_running|:{st.get('port', '?')}"
                    if st["running"]
                    else "γ|boardd|start|fail",
                    flush=True,
                )
            ...
            if st["running"]:
                print(
                    f"γ|boardd|running|:{st.get('port', '?')}|pid={st.get('pid', '?')}",
                    flush=True,
                )
            else:
...
def _board_list(board: JobBoard, args):
    ...
    for t in tasks:
        ...
        print(
            f"γ|board|{icon}|{tag}|{t.id[:8]}|{t.state.value:<12}|{t.caste or '-':<4}|{t.title[:60]}",
            flush=True,
        )
...
def _board_post(board: JobBoard, args):
    ...
    tags = ["tools"] if getattr(args, "tools", False) else None
    t = Task(level=level, title=args.title, prompt=args.prompt or args.title, tags=tags)
    board.create(t)
    print(
        f"γ|board|post|{t.id[:12]}|{t.level.value}|{t.title}{' (tools)' if tags else ''}",
        flush=True,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    infer_p.add_argument(
        "caste",
        choices=["α", "β", "γ", "a", "b", "g", "alpha", "beta", "gamma"],
        help="Caste to infer from (a/b/g or α/β/γ)",
    )
    ...
    patch_p = sub.add_parser(
        "patch", help="Self-mod patch (pipe new content via stdin, dry-run by default)"
    )
    patch_p.add_argument("file", nargs="?", help="Target file to patch")
    patch_p.add_argument(
        "--apply", dest="dry_run", action="store_false", help="Apply the patch"
    )
    patch_p.add_argument(
        "--review", action="store_true", help="Post patch diff to board for review"
    )
    patch_group = patch_p.add_mutually_exclusive_group()
    patch_group.add_argument(
        "--list", action="store_true", help="List known patches/backups"
    )
    ...
    compact_p = sub.add_parser(
        "compact", help="Compact session journal via summarization"
    )
    compact_p.add_argument(
        "--caste",
        choices=["a", "b", "g", "alpha", "beta", "gamma"],
        default="b",
        help="Caste to summarize with (default beta)",
    )
    ...
    board_list_p.add_argument(
        "--level", choices=["epic", "task", "unit"], help="Filter by level"
    )
    board_list_p.add_argument(
        "--state",
        choices=["backlog", "ready", "in_progress", "blocked", "done", "archived"],
        help="Filter by state",
    )

    board_post_p = board_sub.add_parser("post", help="Post a new task to the board")
    board_post_p.add_argument(
        "level", choices=["epic", "task", "unit"], help="Task level"
    )
    board_post_p.add_argument("title", help="Task title")
    board_post_p.add_argument("--prompt", help="Task prompt (defaults to title)")
    board_post_p.add_argument(
        "--tools", action="store_true", help="Enable tool calling for this task"
    )
    ...
    board_claim_p.add_argument(
        "level", choices=["epic", "task", "unit"], help="Level to claim from"
    )
    ...
    board_serve_start_p = board_serve_sub.add_parser(
        "start", help="Start board daemon in background"
    )
    board_serve_start_p.add_argument(
    ...
    sched_add_p.add_argument(
        "when_type", choices=["interval", "daily_at"], help="Trigger type"
    )
    sched_add_p.add_argument("title", help="Schedule title / board task title")
    sched_add_p.add_argument(
        "--every", type=int, default=3600, help="Seconds between ticks (for interval)"
    )
    sched_add_p.add_argument("--at", default="02:00", help="HH:MM time (for daily_at)")
    sched_add_p.add_argument(
        "--level",
        choices=["epic", "task", "unit"],
        default="task",
        help="Board task level",
    )
    sched_add_p.add_argument("--prompt", help="Task prompt text")
    sched_add_p.add_argument(
        "--action",
        choices=["post_to_board"],
        default="post_to_board",
        help="Action type",
    )
    sched_add_p.add_argument(
        "--tools", action="store_true", help="Enable tool calling for scheduled task"
    )
    ...
    announce_p.add_argument(
        "--port", type=int, default=None, help="Daemon port to announce"
    )
    announce_p.add_argument(
        "--castes", type=str, default=None, help="Castes e.g. βγ (default: αβγ)"
    )

    adev_p = sub.add_parser(
        "adev", help="Run adversarial dev mode (programmer + roaster + adjudicator)"
    )
    adev_p.add_argument("task", help="The development task to implement")
    adev_p.add_argument("--workdir", help="Working directory (default: current dir)")
    adev_p.add_argument(
        "--rounds", type=int, default=5, help="Max review rounds (default: 5)"
    )
    adev_p.add_argument(
        "--programmer-caste",
        choices=["a", "b", "g", "alpha", "beta", "gamma"],
        default="gamma",
        help="Caste for programmer (default: gamma)",
    )
    adev_p.add_argument(
        "--roaster-caste",
        choices=["a", "b", "g", "alpha", "beta", "gamma"],
        default="gamma",
        help="Caste for roaster (default: gamma)",
    )
    adev_p.add_argument(
        "--adjudicator-caste",
        choices=["a", "b", "g", "alpha", "beta", "gamma"],
        default="alpha",
        help="Caste for adjudicator (default: alpha)",
    )
    ...
    materialize_p = proj_sub.add_parser(
        "materialize", help="Copy generated project suggestions into working folders"
    )
    materialize_p.add_argument("name", help="Project directory name (partial match)")
    materialize_p.add_argument(
        "--into", help="Destination root (default: ./materialized-projects/<project>)"
    )
    materialize_p.add_argument(
        "--force",
        action="store_true",
        help="Replace existing materialized suggestion folders",
    )
